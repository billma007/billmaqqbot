...
class ArkHeadhunt:
    ...
    def _get_agent_dict() -> dict:
        if not exists('bot_plugin_arknight_addons_agent.json'):
            with open('bot_plugin_arknight_addons_agent.json', 'w+', encoding='utf8') as file:
                dump({}, file, indent=4)

        with open('bot_plugin_arknight_addons_agent.json', 'r', encoding='utf8') as file:
...
# Test
def arknightsmain(setit=2):
    try:
        api = ArkHeadhunt(times=10)
        #print(api.set_up('W', 6))
        returnit="本次六星干员爆率："+str(setit)+"%\n"
        api.get_randomized_results(offset_setting=setit)
